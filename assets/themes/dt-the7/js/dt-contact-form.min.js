jQuery(function ($) {
	...
	var FORMS_ENDPOINT = window.KNPLAW_FORMS_ENDPOINT || '';
	var FORMS_SUBJECT =
	window.KNPLAW_FORMS_SUBJECT ||
	'KNP 홈페이지 문의 | Website inquiry';
	var FORMS_SUCCESS_MESSAGE =
		window.KNPLAW_FORMS_SUCCESS_MESSAGE || '정상적으로 접수되었습니다. We will get back to you shortly.';
	var FORMS_ERROR_MESSAGE =
		window.KNPLAW_FORMS_ERROR_MESSAGE ||
		'메시지 전송에 실패했습니다. 잠시 후 다시 시도하거나 02-552-8381 로 문의해주세요.';

	function submitContactPayload(payload) {
		return new Promise(function (resolve, reject) {
			if (!FORMS_ENDPOINT) {
				reject(new Error('폼 전송 엔드포인트가 설정되지 않았습니다.'));
				return;
			}

			try {
				var formData = new FormData();
				formData.append('_subject', FORMS_SUBJECT);
				...
				if (payload.security_token) {
					formData.append('security_token', payload.security_token);
				}
				...
				if (payload.fields.email) {
					formData.append('_replyto', payload.fields.email);
				} else if (payload.fields.telephone) {
					formData.append('_replyto', payload.fields.telephone);
				}
						...
						return response
							...
							.then(function (body) {
								...
								if (success) {
									resolve({
										success: true,
										message: body.message || FORMS_SUCCESS_MESSAGE
									});
								} else {
									var message =
										(body && (body.message || body.errors)) || FORMS_ERROR_MESSAGE;
		...
		return this.each(function initForm() {
			...
			$form.validationEngine({
				...
				onValidationComplete: function onValidationComplete(form, isValid) {
					...
					submitContactPayload(payload)
						.then(function (response) {
							var $formNode = $(form);
							var message = response && response.message ? response.message : FORMS_SUCCESS_MESSAGE;

							showPrompt($formNode, message, 'pass');
						...
						.catch(function (error) {
							var $formNode = $(form);
							var message = (error && error.message) || FORMS_ERROR_MESSAGE;
							showPrompt($formNode, message, 'error');
							$currentForm.find('.formError').removeClass('field-success');
							captcha.reset();
						});
				}
			});
